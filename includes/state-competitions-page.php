<?php
// includes/state-competitions-page.php - Frontend State Competitions schedule viewer

$stateSchedules = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM schedules WHERE active = 1 AND competition_scope = 'State' ORDER BY start_date ASC, id ASC");
    $stmt->execute();
    $stateSchedules = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    // Fallback empty list
}

// Extract unique event types for filtering
$eventTypes = array_unique(array_filter(array_column($stateSchedules, 'event_type')));
sort($eventTypes);

$page_title = "State Competitions | Boccia India";
$meta_desc = "Official State Boccia Championships, State Ranking Events, and other recognized state-level competitions conducted under the Boccia Sports Federation of India.";
$canonical_url = "page.php?section=competitions&slug=state-competitions";
...
<div class="state-events-page">
    ...
    <section class="board-hero" style="background-image: linear-gradient(90deg, rgba(7, 25, 84, 0.95) 0%, rgba(7, 25, 84, 0.88) 35%, rgba(7, 25, 84, 0.65) 55%, rgba(7, 25, 84, 0.35) 75%, transparent 100%), url('<?php echo htmlspecialchars($relative_prefix); ?>board/board_bg.webp');">
        <div class="container board-hero-container">
            <div class="board-hero-content scroll-reveal">
                ...
                <h1 class="board-hero-title">State Competitions</h1>
                <p class="board-hero-text">
                    Official State Boccia Championships, State Ranking Events, and other recognized state-level competitions conducted under the Boccia Sports Federation of India.
                    <br>
                    <span style="color: var(--boccia-saffron, #FF9933); font-weight: 600;">Participate and compete in state championships.</span>
                </p>
            </div>
        </div>
    </section>
    ...
    <!-- ═══════════ CONTENT SECTION ═══════════ -->
    <section class="state-events-content-section">
        <div class="container">
            <?php if (count($stateSchedules) > 0): ?>
                ...
                <div class="competition-filter-card scroll-reveal">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-md-5">
                            <label for="eventSearchInput" class="filter-label"><i class="bi bi-search me-1"></i> Search Competitions</label>
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <select id="eventTypeFilter" class="form-select rounded-pill py-2" style="font-size: 0.95rem;">
                                <option value="">All Types (<?php echo count($stateSchedules); ?>)</option>
                ...
                <div class="schedule-table-card">
                    ...
                    <div class="schedule-table-wrapper" style="overflow-x: auto; padding-bottom: 1rem;">
                        <div class="schedule-table" style="min-width: 800px;">
                            <div class="schedule-body" id="desktopScheduleContainer" style="display: flex; flex-direction: column; gap: 1.25rem;">
                                <?php 
                                $rowIdx = 1;
                                foreach ($stateSchedules as $sched): 
                    ...
                    <div id="noMatchingEvents" style="display: none; text-align: center; padding: 4rem 1.5rem;">
                        ...
                        <h4 style="color: #081B4B; font-weight: 700; margin-top: 1rem; margin-bottom: 0.5rem;">No Matching Competitions Found</h4>
                        <p style="color: #64748B; margin-bottom: 1.5rem;">No state competitions match your current search and filter criteria.</p>
            ...
            <?php else: ?>
                <div class="schedule-table-card" style="text-align: center; padding: 5rem 2rem;">
                    <h4 style="color: var(--boccia-navy, #081B4B); font-weight: 700; margin-bottom: 0.5rem;">No State Competitions Scheduled</h4>
                    <p style="color: var(--boccia-text-muted, #64748B); margin-bottom: 0;">Please check back later for updates on upcoming state-level championships.</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
