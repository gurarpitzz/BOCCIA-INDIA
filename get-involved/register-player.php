...
async function submitApplication() {
    ...
    try {
        ...
        const medFile = await prepareFile(document.getElementById("file_med")?.files[0]);

        // Check total payload size for remaining uncompressed files (e.g. large PDFs)
        const totalPayload = calculateTotalPayloadSize([photoFile, docFile, medFile]);
        ...
        if (medFile) {
            fd.set("medical_certificate", medFile);
        }

        const res = await fetch("../api/player-registration.php", {
            method: "POST",
            body: fd
        });
