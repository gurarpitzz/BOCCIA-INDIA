...
import { ArrowLeft, ArrowRight, CheckCircle, UploadCloud, RotateCcw } from 'lucide-react';

const DRAFT_KEY = 'bsfi_player_registration_draft';
...
const step2Schema = z.object({
  age_category: z.string().min(1, 'Please select age category'),
...
export default function PlayerRegistration() {
  ...
  const onSubmitForm = async () => {
    if (!photoFile || !docFile) {
      setErrorMsg('Please upload both required photo and passport documents.');
      return;
    }
    ...
    try {
      const res = await fetch('/registration/api/register/player', {
        method: 'POST',
        body: fd
      });
      const data = await res.json();
    ...
    <main className="min-h-screen bg-slate-950 text-white flex flex-col justify-center items-center p-4">
      ...
      <div className="max-w-2xl w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-12 shadow-2xl space-y-8">
        <header className="text-center space-y-2">
          <h1 className="text-3xl md:text-4xl font-extrabold text-white">Player Registration</h1>
        ...
        {step >= 1 && step <= 3 && (
          <div className="space-y-3">
            <div className="flex justify-between text-xs font-semibold text-slate-400 uppercase tracking-wider">
              ...
              <span>{step === 1 ? 'Personal Details' : step === 2 ? 'Impairment & Size' : 'Identity Verification'}</span>
            </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Father's Name</label>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Mother's Name</label>
                  <input type="text" {...register('mother_name')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl" />
          ...
          {step === 2 && (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Age Category</label>
                  <select {...register('age_category')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl">
                    <option value="">Select Age Category</option>
                    <option value="Junior (U18)">Junior (U18)</option>
                    <option value="Senior">Senior</option>
                  </select>
                  {errors.age_category && <p className="text-red-500 text-xs mt-1">{errors.age_category.message}</p>}
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Impairment Type</label>
                  <input type="text" placeholder="e.g. Cerebral Palsy" {...register('impairment_type')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl" />
                  {errors.impairment_type && <p className="text-red-500 text-xs mt-1">{errors.impairment_type.message}</p>}
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Boccia Category</label>
                  <select {...register('classification')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl">
                  ...
                  {errors.classification && <p className="text-red-500 text-xs mt-1">{errors.classification.message}</p>}
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Wheelchair Usage</label>
                  <select {...register('wheelchair_status')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl">
                    <option value="">Select Usage</option>
                    <option value="Yes">Yes</option>
                    <option value="No">No</option>
                  </select>
                  {errors.wheelchair_status && <p className="text-red-500 text-xs mt-1">{errors.wheelchair_status.message}</p>}
                </div>
                <div>
                ...
                <div>
                  ...
                  <select {...register('kit_tracksuit')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl">
                    <option value="">Select Track Suit Size</option>
                    <option value="XS">XS</option>
                    <option value="S">S</option>
                    <option value="M">M</option>
                    <option value="L">L</option>
          ...
          {step === 3 && (
            ...
            >
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Aadhaar Card Number</label>
                  <input type="text" maxLength={12} {...register('aadhaar')} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Passport PDF / Scan Copy</label>
                    <div className="border-2 border-dashed border-slate-800 hover:border-orange-500 rounded-2xl p-6 text-center cursor-pointer transition relative">
                      <input
                        type="file"
                      ...
                      />
                      <UploadCloud className="mx-auto mb-2 text-slate-500" size={32} />
                      <span className="text-sm font-semibold">{docFile ? docFile.name : 'Choose Document File'}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {step === 4 && regResult && (
            ...
            >
              ...
              <p className="text-slate-400 text-sm max-w-md mx-auto">Your application has been received and is currently under review by the BSFI officials.</p>
